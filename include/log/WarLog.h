...
struct Esc
{
    ...
    const boost::string_ref& GetVal() const { return val_; }
...
/*! The log event handler interface
    ...
    \note Any constructor or method may throw war::ExceptionBase derived exceptions
*/

class LogEventHandler
{
public:
    typedef std::shared_ptr<LogEventHandler> ptr_t;
    struct Exception : public war::ExceptionBase {};

    LogEventHandler(const std::string& name, const LogLevel level, const filter_t filter)
    ...
    const filter_t GetFilter() const { return filter_; }

    static const char *GetLevelName(const LogLevel level);

    static void WriteFilter(std::ostream& out, const filter_t filter, const filter_t hide = LA_GENERAL);

	void SetLevel(const LogLevel level) { level_ = level; }

protected:
    /*! This is the default implementation for formatting output to a log-device. */
    void WriteDefaulInfo(std::ostream& out, const SubmitInfo &si) const;
    void WriteLevel(std::ostream& out, const SubmitInfo &si) const;
    void WriteFilter(std::ostream& out, const SubmitInfo &si) const;
    void WriteTimestamp(std::ostream& out, const SubmitInfo &si) const;
    ...
    */
    void WriteMessage(std::ostream& out, const SubmitInfo &si) const;
...
public:
	...
	virtual void Submit ( const SubmitInfo& info ) { WriteDefaulInfo(out_, info); }
...
/*! The log-manager.
  ...
  There most be one and only one instace of this object in an application
...
public:
    ...
    static bool IsRelevant (const LogLevel level, const filter_t filter) {
    ...
    */
    static LogLevel GetLevelFromName(const std::string& name);

private:
    void DoSubmit ( Log& log );
...
{
public:
    Log (const LogLevel level, const filter_t filter) throw()
            : level_ (level), filter_ (filter) {}

	Log& operator = (Log &&log) = delete;
	Log& operator = (const Log &log) = delete;

    ~Log() { LogEngine::Submit ( *this ); }

    operator bool () const { return true; }

    std::ostringstream& Get() { return buf_; }

    LogLevel GetLevel() const { return level_; }

    filter_t GetFilter() const { return filter_; }
...
struct Errno
{
	...
	Must be constructed after an system-error occurs, and before
	any new system calls are made.
    */
...
public:
    LogFunctionCall(const char *functionName,
        const int line,
        const char *file,
        const LogLevel level = war::log::LL_TRACE3)
        : name_(functionName), file_(file), line_(line), level_(level)
    {
        __WAR_LOG_WITH_LEVEL_AND_FILTER(level_, LA_FUNCTION_CALL) << "Entering: " << name_ << ' ' << file_ << ' ' << line_;
    }

    ~LogFunctionCall()
    {
        __WAR_LOG_WITH_LEVEL_AND_FILTER(level_, LA_FUNCTION_CALL) << "Leaving: " << name_ << ' ' << file_ << ' ' << line_;
    }
