...
/*! Single-thread task sequencer, timer and asio io_service instance
	...
	the objects that belong to the connection is single-threaded,
	as long as we only access its properties trough the task-sequencer.
	This simplifies multithreading a bit, and reduce the need for
...
public:
	//using ptr_t = std::shared_ptr<Pipeline>;
	//using weak_ptr_t = std::weak_ptr<Pipeline>;
	...
	*/
	...
	void Post(const func_t& fn, const char * name);
	void Post(func_t&& fn, const char * name);
	...
	*/
	...
	void Dispatch(const func_t& fn, const char * name);
	void Dispatch(func_t&& fn, const char * name);
	...
	*/
	...
	void PostWithTimer(const func_t& fn, const char * name, const std::uint32_t milliSeconds);
	void PostWithTimer(func_t&& fn, const char *, const std::uint32_t milliSeconds);

	/*! Returns the asio io-service */
	io_service_t &GetIoService() {
	...
	*/
	...
	bool IsPipelineThread() const { return thread_->get_id() == std::this_thread::get_id(); }
