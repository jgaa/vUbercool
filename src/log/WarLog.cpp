...
namespace war { namespace log {
    ...
    void LogEngine::DoSubmit ( Log& log )
    {
    ...
    void LogEngine::UpdateLevelAndFilter()
    {
        ...
        LOG_DEBUG << "Log-level is now " << current_level_ << " and filter is " << filters.str() << '(' << current_filter_ << ')';
    }

	LogLevel LogEngine::GetLevelFromName(const std::string& name)
    {
    ...
    void LogEventHandler::WriteLevel(std::ostream& out, const SubmitInfo &si) const
    {
        out << si.level_;
    }

    void LogEventHandler::WriteFilter(std::ostream& out, const SubmitInfo &si) const
    {
        WriteFilter(out, si.filter_);
    }

    void LogEventHandler::WriteFilter(std::ostream& out, const filter_t filter, const filter_t hide)
    {
        bool virgin = true;
#define ADD(name) if ((filter & (LA_ ## name & ~hide)) == LA_ ## name) { if (!virgin) {out << "|"; } out << #name; virgin=false;}
        ADD(GENERAL)
        ADD(SECURITY)
        ADD(TRANSFER)
        ADD(AUTH)
...
#undef ADD
    ...
    void LogEventHandler::WriteTimestamp(std::ostream& out, const SubmitInfo &si) const
	{
		std::tm my_tm{};
...
#ifdef _MSC_VER
		localtime_s(&my_tm, &when);
#else
		localtime_r(&when, &my_tm);
#endif
		if (my_tm.tm_year) {
...
// 				<< std::setw(3) << std::setfill('0') << milliseconds;
    ...
    void LogEventHandler::WriteMessage(std::ostream& out, const SubmitInfo &si) const
    {
    ...
    const char *LogEventHandler::GetLevelName(const LogLevel level)
    {
        static const char *names[] = { "FATAL", "ERROR", "WARN", "INFO", "NOTICE",
            "DEBUG", "TRACE1", "TRACE2", "TRACE3", "TRACE4" };
    ...
    void LogEventHandler::WriteDefaulInfo(std::ostream& out, const SubmitInfo &si) const
    {
	...
	LogToFile::LogEventHandler::ptr_t LogToFile::Create(const path_t& path,
		...
		const filter_t filter)
	{
		return LogEventHandler::ptr_t(new LogToFile(path, truncateFileOnOpen, name, level, filter));
	}
