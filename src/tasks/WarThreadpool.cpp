...
war::Threadpool::Threadpool(const unsigned numThreads, unsigned maxPerThreadQueueCapacity)
: capacity_(numThreads > 0 ? numThreads : max<unsigned>(2, thread::hardware_concurrency() - 1))
{
	...
	for (unsigned i = 0; i < capacity_; ++i) {
		...
		pool_.emplace_back(std::unique_ptr<Pipeline>
			(new Pipeline{name, static_cast<int>(i), maxPerThreadQueueCapacity}));
	}
